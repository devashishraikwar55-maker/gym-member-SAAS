...
export function DashboardView({ 
  ...
  return (
    <div id="dashboard-view-container" className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-[#6B7280]">
            {totalCount === 0 
              ? 'Start managing your gym members, plan renewals, and WhatsApp alerts.' 
      ...
      {totalCount === 0 ? (
        /* Empty State: Center of screen with prominent [Add First Member] button */
        <div 
          id="dashboard-empty-state"
          className="w-full flex flex-col items-center justify-center min-h-[460px] bg-white border border-[#ECEEF3] rounded-[32px] p-8 sm:p-14 shadow-sm text-center relative overflow-hidden my-4"
        >
          {/* Subtle Ambient Glow */}
          <div className="absolute -top-20 left-1/2 -translate-x-1/2 w-96 h-96 bg-indigo-100/50 rounded-full blur-3xl -z-0 pointer-events-none" />

          <div className="relative z-10 max-w-lg mx-auto flex flex-col items-center">
            {/* Attractive Icon Badge */}
            <div className="relative mb-5">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-tr from-brand-primary to-indigo-600 flex items-center justify-center text-white shadow-xl shadow-indigo-500/25">
                <UserPlus className="w-10 h-10 stroke-[2.2]" />
              </div>
              <div className="absolute inset-0 rounded-2xl bg-indigo-400 blur-lg opacity-30 -z-10" />
            </div>

            <h2 className="text-2xl sm:text-3xl font-black text-brand-text-primary tracking-tight">
              No Members Added Yet
            </h2>
            <p className="text-xs sm:text-sm text-brand-text-secondary mt-2 leading-relaxed max-w-md">
              Your gym directory is currently empty. Add your first member to start tracking active plans, renewal countdowns, and automated reminders.
            </p>

            {/* Requested [Add First Member] Button in the Middle */}
            <div className="mt-7">
              <button
                id="dashboard-add-first-member-btn"
                onClick={() => onNavigate('add-member')}
                className="inline-flex items-center justify-center gap-2.5 px-8 py-4 bg-brand-primary text-white font-bold rounded-2xl hover:bg-brand-primary-hover active:scale-[0.98] transition-all shadow-lg shadow-indigo-500/30 text-base cursor-pointer group"
              >
            ...
            {/* Quick 3 Feature Highlights */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 w-full mt-10 text-left">
              <div className="p-3.5 bg-gray-50/80 rounded-2xl border border-gray-100/80">
                <div className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-brand-primary" />
                  1. Member Profile
                </div>
                <div className="text-[11px] text-gray-500 mt-1">Name, phone, photo, & membership tier</div>
              </div>
              <div className="p-3.5 bg-gray-50/80 rounded-2xl border border-gray-100/80">
                <div className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                ...
                <div className="text-[11px] text-gray-500 mt-1">Track memberships expiring in 7 days</div>
              </div>
              <div className="p-3.5 bg-gray-50/80 rounded-2xl border border-gray-100/80">
                <div className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                ...
                <div className="text-[11px] text-gray-500 mt-1">Quick WhatsApp reminder messages</div>
              </div>
            </div>
          </div>
        </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="text-right flex items-center gap-2">
                            ...
                            <a
                              id={`dash-whatsapp-send-${member.id}`}
                              href={getWhatsAppRenewUrl(member.phone, member.name, 'GYM-member', member.expiryDate, member.duration)}
                              target="_blank"
                              rel="noopener noreferrer"
                              onClick={(e) => e.stopPropagation()}
                              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 hover:text-emerald-800 border border-emerald-200/80 transition-all active:scale-95 cursor-pointer shadow-2xs"
